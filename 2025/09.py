...
for i,child in enumerate(sequences.values()):
    parents = [d for j,d in enumerate(sequences.values()) if i != j]
...
total = 0
for child,*parents in permutations(sequences.values(),3):
...
for child,*parents in permutations(sequences.items(),3):
    ...
    if parents[0][0] > parents[1][0]: continue # Skip duplicates
