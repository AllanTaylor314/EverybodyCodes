...
def load_file(part):
    ...
    return {
        (r, c): cell == "#" for r, row in enumerate(grid) for c, cell in enumerate(row)
    }
...
# but then it's also even and symmetrical, so that doesn't help that much
...
target_states = load_file(3)
assert len(target_states) == 8 * 8
...
print(
    sum(matches.values()) * num_reps + sum(v for k, v in matches.items() if k < extra)
)
